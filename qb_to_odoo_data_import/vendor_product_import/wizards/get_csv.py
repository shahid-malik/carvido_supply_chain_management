...
class WizardGetFile(models.TransientModel):
    ...
    def import_quickbooks_saleorder_data(self, row):
        ...
        order_no = str(row["Auftragsnummer"]) if self.check_is_nan(row["Auftragsnummer"]) is False else None
        expected_shipping_date = datetime.datetime.strptime(str(row["Auslieferungsdatum"]), "%m/%d/%Y").strftime(
            "%Y-%m-%d") if self.check_is_nan(row["Auslieferungsdatum"]) is False else None
        ...
        expected_delivery_date = datetime.datetime.strptime(str(row["Anlieferungsdatum"]), "%m/%d/%Y").strftime(
            "%Y-%m-%d") if self.check_is_nan(row["Anlieferungsdatum"]) is False else None

        customer = self.env['res.partner'].sudo().search([('name', '=', customer_name)])
        if not customer:
            customer = self.env['res.partner'].sudo().create({'name': customer_name})
        ...
        if math.isnan(row['Quantity']):
            quantity = 0
        express = str(row["Express"]) if self.check_is_nan(row["Express"]) is False else None
        if express == 'X':
            express = None
        product_name = str(row["Produkt ID"]) if self.check_is_nan(row["Produkt ID"]) is False else None
    ...
    def import_quickbooks_invoice_data(self, row):
        ...
        product_dict = {}
        Create_product_name = row['GroupLineItemFullName'] if self.check_is_nan(
            row['GroupLineItemFullName']) is False else None
