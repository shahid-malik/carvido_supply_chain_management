...
from odoo import models, fields, tools, _, SUPERUSER_ID, api
from odoo.exceptions import ValidationError, UserError


class MediodProductTemplate(models.Model):
    _name = "mediod.product.template"

    is_custom_field = fields.Binary(string="Custom")
    ...
    taxes_id = fields.Many2many('account.tax', 'product_taxes_rel_temp', 'prod_id_temp', 'tax_id_temp',
    ...
    invoice_policy = fields.Selection([
        ...
        default='order')
    product_image_1 = fields.Char('Image Medium')
    supplier_taxes_id = fields.Many2many('account.tax', 'product_supplier_taxes_rel_temp', 'prod_id_temp',
                                         'tax_id_temp',
    ...
    packaging_ids = fields.One2many(
        'product.packaging', string="Product Packages", compute="_compute_packaging_ids",
    ...
    has_configurable_attributes = fields.Boolean("Is a configurable product",
                                                 compute='_compute_has_configurable_attributes', store=True)
    ...
    #     for template in self:
    #         template.can_image_1024_be_zoomed = template.image_1920 and tools.is_image_size_above(
    #             template.image_1920, template.image_1024)
    ...
    def _compute_has_configurable_attributes(self):
        ...
        """
        for product in self:
            product.has_configurable_attributes = product.has_dynamic_attributes() or any(
                len(ptal.value_ids) >= 2 for ptal in product.attribute_line_ids)
    ...
    def _compute_template_price_no_inverse(self):
        ...
        if pricelist_id_or_name:
            ...
            if isinstance(pricelist_id_or_name, str):
                pricelist_data = self.env['product.pricelist'].name_search(pricelist_id_or_name, operator='=',
                                                                           limit=1)
                if pricelist_data:
    ...
    def _set_template_price(self):
        if self._context.get('uom'):
            for template in self:
                value = self.env['uom.uom'].browse(self._context['uom'])._compute_price(template.price,
                                                                                        template.uom_id)
    ...
    def _check_uom(self):
        if any(
                template.uom_id and template.uom_po_id and template.uom_id.category_id != template.uom_po_id.category_id
                for template in self):
            raise ValidationError(
                _('The default Unit of Measure and the purchase Unit of Measure must be in the same category.'))
        return True
    ...
    #             template_ids = super(ProductTemplate, self)._name_search(name=name, args=domain_template,
    ...
    def has_dynamic_attributes(self):
        ...
        self.ensure_one()
        return any(
            a.create_variant == 'dynamic' for a in self.valid_product_template_attribute_line_ids.attribute_id)
    ...
    #             else:
    #                 result[product_attribute_value.id] = filter_line.product_tmpl_id.mapped(
    #                     'attribute_line_ids.product_template_value_ids').ids
    #
    #     return result
    #
    ...
    #         if log_warning:
    #             _logger.warning('The user #%s tried to create an invalid variant for the product %s.' % (
    #             self.env.user.id, self.id))
    #         return Product
    #
    ...
    #
    #     return self.env['product.product'].sudo().with_context(active_test=False).search(domain,
    #                                                                                      order='active DESC',
    #                                                                                      limit=1).id
    #
    ...
    #     resulting empty combination is actually possible or not.
    #     """
    #     return next(self._get_closest_possible_combinations(combination),
    #                 self.env['product.template.attribute.value'])
    ...
    #     }]
    #
    # call on click of approve Button
    def approve_product_template(self):
        mediod_keys = list(self._fields.keys())
        vals = {}
        vals = {
            'name': self.name,
            'asin': self.asin,
            ...
            'ws_each': self.ws_each,
            'ws_case': self.ws_case,
            'msrp_each': self.msrp_each,
            'msrp_case': self.msrp_case,
            'length': self.length,
            'width': self.width,
            'height': self.height,
            'ctn_weight': self.ctn_weight,
            'formulation_sg': self.formulation_sg,
            'rounded_kg': self.rounded_kg,
            'gross_weight': self.gross_weight,
            'container_weight_lbs': self.container_weight_lbs,
            'rounded_lbs': self.rounded_lbs,
            'liquid_metric': self.liquid_metric,
            'weight_metric': self.weight_metric,
            'weight_imp_lbs': self.weight_imp_lbs,
            'product_image': self.product_image,
            'image_1920': self.image_1920,
            'liquid_imp': self.liquid_imp,
            'image_metaname': self.image_metaname,
            'sale_ok': self.sale_ok,
            'purchase_ok': self.purchase_ok,
            'sh_extra_image_url': self.sh_extra_image_url,
            # 'image_1920': self.image_1920,
        }
        create_data = self.env['product.template'].create(vals)
        if create_data:
            ...
            result = {
                ...
                'type': 'ir.actions.act_window',
                'view_mode': 'tree,form',
                'res_model': 'mediod.product.template'
            }
            return result
